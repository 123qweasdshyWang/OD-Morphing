...
{
  std::fstream fout;
  fout.open("waypoints.txt", std::fstream::out);
  
  this->minCutSolver.GetCuts();

  using std::vector;

  bool prevIsCut = false;
  size_t prevNextPId = -1;
  for (size_t lineId = 0; lineId < this->rawLines.size(); lineId ++) {
    ...
    for (size_t i = 0; i < cuts.size(); i ++) {
      ...
      if (i == 0) {
        ...
        oId = 0;
        //oId = line.FindPointIndexById(rawLine[cuts[i - 1]].id);
      }
      if (i == cuts.size() - 1) {
        dId = line.GetPointSize() - 1;
      } else {
        dId = line.GetPointSize() - 1;
        //dId = line.FindPointIndexById(rawLine[cuts[i + 1]].id);
      }

      Point & rawPoint = rawLine[i];
      const Point &rp = refPoints[rawPoint.id];
      Waypoint wp(rp.x, rp.y, rp.id);
      line.AddWaypoint(oId, dId, wp);
      fout << lineId << " " << (rp.x * (this->xMax - this->xMin) + this->xMin) << " " << (rp.y * (this->yMax - this->yMin) + this->yMin) << std::endl;
			//std::cout << lineId << " " << oId << " " << dId << std::endl;
    }
  }
