...
void loadOutputToDB()
{
    std::string command = "/***/***/***/***//miniconda3/envs/py3/bin/python monitorFileSystemChange.py";
...
private:
    ...
    void startAnimation(const Rest::Request& request, Http::ResponseWriter response) {
        ...
        char fileName[50];
        sprintf(fileName, "%.3f_%.3f_%d.json", double_param[0], double_param[1], int_param[0]);
    ...
    void morphingInRealTime(const Rest::Request& request, Http::ResponseWriter response) {
        ...
        char fileName[50];
        sprintf(fileName, "%.3f_%.3f_0.json", double_param[0], double_param[2]);
