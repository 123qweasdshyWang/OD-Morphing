...
int Application::Input(int layerNum, double maxLng, double minLng, double maxLat, double minLat)
{
    ...
    // read nodes
    {
        char file1[100] = "/***/***/***/***/Nodes.csv";
        //sprintf(file1, "/***/***/***/***/nodes.txt", this->inputPath);
        fstream nodeIn;
	...
	client.connect(session, user_name, passwd, db_name);  // connect to mapd_server
        ...
        std::string query = "select sequence from Mapped_TaxiNum5000_1_Aug3_Morning where (startLng <= "+std::to_string(maxLng)+" and startLng >= "+std::to_string(minLng)+" and startLat <= "+std::to_string(maxLat)+" and startLat >= "+std::to_string(minLat)+") and (endLng <= "+std::to_string(maxLng)+" and endLng >= "+std::to_string(minLng)+" and endLat <= "+std::to_string(maxLat)+" and endLat >= "+std::to_string(minLat)+") order by rowid";
        //std::string query = "select sequence from Mapped_TaxiNum5000_1_Aug3_Morning where rowid in (27, 29, 118) order by rowid;";
	...
	vector<std::string> nodeSequence_vec = nodeSequence.data.str_col;
        ...
        this->dataSet.roads.resize(500000);
